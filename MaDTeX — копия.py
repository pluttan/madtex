...
class Replacer:
    ...
    def APsection(self, teg):
        if teg.get("only_teg") == "h2":
            p = teg.get("all_teg").replace(
                "<"+teg.get("teg")+">", "").replace("</h2>", "")
            self.file = self.file.replace(teg.get("all_teg"),
                                          r"\section*{"+p +
                                          r"}\addcontentsline{toc}{section}{"+p +
                                          r"}", 1)
        if teg.get("only_teg") == "h3":
            p = teg.get("all_teg").replace(
                "<"+teg.get("teg")+">", "").replace("</h3>", "")
            self.file = self.file.replace(teg.get("all_teg"),
                                          r"\subsection*{"+p +
                                          r"}\addcontentsline{toc}{subsection}{"+p +
                                          r"}", 1)
        if teg.get("only_teg") == "h4":
            p = teg.get("all_teg").replace(
                "<"+teg.get("teg")+">", "").replace("</h4>", "")
            self.file = self.file.replace(teg.get("all_teg"),
                                          r"\subsubsection*{"+p +
                                          r"}\addcontentsline{toc}{subsubsection}{"+p +
                                          r"}", 1)
    ...
    def findTeg(self, i: int) -> tuple:
        ...
        return {
            ...
            "is_title": tegsp[0] == "h1",
            "is_section": tegsp[0] == "h2" or tegsp[0] == "h3" or tegsp[0] == "h4",
            "out": i,
            "out_out": i+len(teg),
            "teg_out": "/"+tegsp[0],
